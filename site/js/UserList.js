    ...
    async componentDidMount() {
        ...
        if (typeof this.state.created_users !== 'undefined' && this.state.created_users.length > 0) {
            this.intervalId1 = setInterval(async () => {
                ...
                await this.update_lists(users, "join request");
            }, 500);
        }
        if (typeof this.state.suspended_users !== 'undefined' && this.state.suspended_users.length > 0) {
            this.intervalId2 = setInterval(async () => {
                ...
                await this.update_lists(users, "suspended users");
            }, 500);
        }
        if (typeof this.state.active_users !== 'undefined' && this.state.active_users.length > 0) {
            this.intervalId3 = setInterval(async () => {
                ...
                await this.update_lists(users, "suspend or delete");
            }, 500);
        }
    }
    ...
    render() {
        return React.createElement(
            ...
            React.createElement(
                "div",
                null,
                this.props.active === "only_active_users" ? React.createElement(
                    "h1",
            ...
            typeof this.state.created_users !== 'undefined' && this.state.created_users.length > 0 ? this.state.created_users.map((item, index) => {
                if (item.name !== "Root") return React.createElement(UserItem, { name: item.name, id: item.id, token: this.state.tok, request: this.state.req, key: index });
            }) : "",
            typeof this.state.suspended_users !== 'undefined' && this.state.suspended_users.length > 0 ? this.state.suspended_users.map((item, index) => {
                if (item.name !== "Root") return React.createElement(UserItem, { name: item.name, id: item.id, token: this.state.tok, request: this.state.req, key: index });
            }) : "",
            typeof this.state.active_users !== 'undefined' && this.state.active_users.length > 0 ? this.state.active_users.map((item, index) => {
                if (item.name !== "Root" && this.props.active === "suspend_or_delete_list" && this.props.request === "suspend or delete") return React.createElement(UserItem, { name: item.name, id: item.id, active: "", token: this.state.tok, request: this.state.req, key: index });else if (item.name !== "Root" && this.props.active === "only_active_users" && this.props.request === "suspend or delete") return React.createElement(UserItem, { name: item.name, id: item.id, active: "only_active_users", token: this.state.tok, request: this.state.req, key: index });
