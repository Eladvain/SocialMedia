class PostsList extends React.Component {
    ...
    async componentDidMount() {
        ...
        this.posts_interval_id = setInterval(() => {
    ...
    render() {
        return React.createElement(
            ...
            React.createElement(
                'div',
                { className: 'new_post_label' },
                React.createElement(
                    'button',
                    { className: 'postList_indicator', type: 'submit', onClick: this.handle_new_posts_update },
                    'new Posts counter: ',
                    this.state.interval_posts_diff
                )
            ),
            React.createElement(
                'h1',
            ...
            React.createElement(
                ...
                this.state.posts.map((post, index) => {
                    return React.createElement(Post, { post_text: post.text, writer: post.writer_user_name, creation_date: post.date, key: index });
                })
            )
        );
    }
