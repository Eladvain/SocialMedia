    ...
    render() {
        return (
            <div className="send_message_container">
                    <div className="send_to_label_and_input">
                                <form>
                                    <label>Choose a friend:</label>
                                    <select name="sent_to" onChange={this.handle_choose_friend} required>
                                        {this.state.user_user_id_dict.map((user, index) =>
                                            <option value={user.id} name={user.user_id } key={user.user_id}>{user.user_name }</option>
                                        )}
                                    </select>
                        <div className="send_to_label_and_input">
                            <label>text:</label>
                            <textarea placeholder="please add the message here" value={ this.state.message_text} onChange={ this.hande_message_text_change} required/>
                        </div>
                        <div className="send_to_label_and_input">
                            <button onClick={this.handle_send_message}>SEND</button>
                            </div>
                            </form>   
                    </div>
            </div>
