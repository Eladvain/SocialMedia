class MessagesList extends React.Component 
{
    ...
    render() {
        return <div>
           <label className="messages_list_label">Messages</label><br></br>
              {this.state.messages.map((message, index) => {
                return  <MessageItem className = "messageItem" key = {index} text = {message.text} send_from_id_user = {message.id_from_user_name} date = {message.date}/>                      
              }  ) }
           </div>
	}
} 